            ...
            <div className="flex-1 bg-dark/50 backdrop-blur-md rounded-2xl shadow-inner p-4 flex flex-col justify-between overflow-hidden border border-white/10">
                    ...
                    <div className="messages-area flex-1 overflow-y-auto pr-2 space-y-4">
                        {messages.map((msg, index) => (
                            <div key={index} className={`flex items-end gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                ...
                                <div className={`max-w-xs md:max-w-md lg:max-w-lg p-4 rounded-2xl shadow ${msg.role === 'user' ? 'bg-secondary text-dark rounded-br-none' : 'bg-white dark:bg-slate-700 text-dark dark:text-light rounded-bl-none'}`}>
                ...
                <div className="input-area mt-4 flex gap-3">
                    ...
                    <button
                        ...
                        className="w-14 h-14 bg-secondary text-dark rounded-full text-xl flex items-center justify-center transition-transform hover:scale-110 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
